  ...
  return createElysia({ prefix: "/posts" })
    ...
    .post(
      ...
      async ({ body }) => {
        if (body.type === ProofType.PROMOTE_POST) {
          await getQueue(QueueName.PromotePost).add(
            `${body.type}-${Date.now()}`,
            body
          );
        } else {
          await getQueue(QueueName.Default).add(
            `${body.type}-${Date.now()}`,
            body
          );
        }
    ...
    .post(
      ...
      async ({ body, submitHashBackend }) => {
        ...
        await createPostMapping(params.hash, undefined, bestOfResponse.hash);
    ...
    .post(
      ...
      async ({ body }) => {
        ...
        await neynar.post({
          text: `REVEALED: Posted by ${
            username ? `@${username}` : `${address}`
          }`,
          embeds: [`https://rumourcast.xyz/posts/${body.castHash}`],
          quote: body.castHash,
